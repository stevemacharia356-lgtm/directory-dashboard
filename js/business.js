...
async function saveBusiness(status, existingSlug) {
  ...
  try {
    ...
    if (!name || !location || !slug) { alert('Business name, location, and slug are required.'); if (btnDraft) { btnDraft.disabled = false; btnDraft.textContent = 'Save as Draft'; } if (btnPublish) { btnPublish.disabled = false; btnPublish.textContent = 'Save & Publish'; } return; }
